...
model.variables.add(names=alpha_names, lb=[0] * n)
model.variables.add(names=beta_names, lb=[0] * n)
...
objective = [(alpha_names[i], c_minus[i]) for i in range(n)] + [(beta_names[i], c_plus[i]) for i in range(n)]
model.objective.set_linear(objective)

# Étape 3: Ajout des contraintes
for i in range(n):
    # Contrainte (3): Xi - Ti = αi - βi (Spécifique au problème 1)
    ...
    val = [1.0] * m
    model.linear_constraints.add(
        lin_expr=[cplex.SparsePair(ind=ind, val=val)],  # Somme(r=1 à m) de γir 
        senses=["E"],                                   # ==
        rhs=[1]                                         # 1
    )
